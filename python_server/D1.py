...
# 配置参数D1 - 三维动态
truePosD1 = np.array([[0, 3.2, 0]]) # 2维定位纵坐标设置为0
dir = 'logData/'
log_file = 'D1.txt'
filePathD1 = dir + log_file

# 读取数据文件 定位解算
dataD1 = dataAna(truePosD1, filePathD1)
posLS = dataD1.calPos()
...
rmsLS = dataD1.calRMS_dynamic(tracePoint=tracePoint, axis=[0,1,2])

# posEKF = dataD1.calPosEKF()
# rmsEKF = dataD1.calRMS3D()
...
# 真实位置和定位结果
plt.plot([-1, 12], [dataD1.truePos[0,1], dataD1.truePos[0,1]],
...
plt.grid('on')
rmsLS = dataD1.calRMS_dynamic(tracePoint=tracePoint, axis=[0,1,2])
rmsLSxy = dataD1.calRMS_dynamic(tracePoint=tracePoint, axis=[0,1])
rmsLSz = dataD1.calRMS_dynamic(tracePoint=tracePoint, axis=[2])
plt.title('场景D1定位测试图，定位结果RMS=%.4f米\n'
          '水平RMS=%.4f米,高度RMS=%.4f米'%(rmsLS, rmsLSxy, rmsLSz))
plt.show()
fig.savefig(figPath+'D1.svg',format='svg',dpi=150)#输出